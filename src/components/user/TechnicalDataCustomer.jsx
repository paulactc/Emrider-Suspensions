import { NavLink } from "react-router";
function TechnicalDataCustomer() {
  return (
    <>
      <section className="body-list-client">
        <h1 className="title-Shock">SHOCK SPEC.SHEET</h1>
      </section>
      <NavLink className="Newcustom" to="/FormTechnicalDataCustom">
        Crear nuevos datos tecnicos
      </NavLink>
      <section className="subsection">
        <h3 className="subtitles">BIKE</h3>
        <ul className="Sublists">
          <li>
            <p className="sizeli">Brand: Yamaha</p>
            <p className="sizeli">Model: Versys 650</p>
            <p className="sizeli">Year: 2017</p>
            <p className="sizeli">Speciality: Pdte rellenar</p>
          </li>
        </ul>
      </section>

      <section className="subsection">
        <h3 className="subtitles">Mantenimiento de tus suspensiones:</h3>
        <ul className="Sublists">
          <li>
            <p className="sizeli">Km moto: 138.000Km</p>
            <p className="sizeli">Fecha próxima mantenimiento: 15/02/26</p>
          </li>
        </ul>
      </section>

      <section className="subsection">
        <h3 className="subtitles">SUSPENSION</h3>
        <ul className="Sublists">
          <li>
            <p className="sizeli">Marca: Showa</p>
            <p className="sizeli">Model: Pdte rellenar</p>
            <p className="sizeli">Year; 2017</p>
          </li>
        </ul>
      </section>

      <section className="subsection">
        <h3 className="subtitles">SETTING FINAL-SHOCK</h3>
        <ul className="Sublists">
          <li>
            <p className="sizeli">Spring: 16 N/mm</p>
            <p className="sizeli">Spec: 25-00198</p>
            <p className="sizeli">Initial: 45</p>
            <p className="sizeli">Oil: 1330 Öhlins</p>
            <p className="sizeli">Gas: 12kgf/cm2</p>
            <p className="sizeli">Rebound: </p>
            <p className="sizeli">Comp.Low: 15</p>
            <p className="sizeli">Comp.High: 25</p>
            <p className="sizeli">Rebound spring: 58n/mm</p>
            <p className="sizeli">Stroke: 63mm</p>
            <p className="sizeli">Height RR: 89mm</p>
          </li>
        </ul>
      </section>

      <section className="subsection">
        <h4 className="subtitles">REMARKS:</h4>
        <ul>
          <li>
            <p className="sizeli">La precarga se ajusta al peso del piloto</p>
          </li>
        </ul>
      </section>
    </>
  );
}

export default TechnicalDataCustomer;
